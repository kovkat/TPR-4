...
#     print('Множина Неймана-Моргенштерна:', Q[-1])
# else:
#     print('\nАциклічне: False')
...
# Масиви для збереження результатів
d_values = np.arange(0.05, 0.55, 0.05)  # Значення d від 0 до 0.5 з кроком 0.05
c_values = np.arange(0.5, 1.05, 0.05)   # Значення c від 0.5 до 1 з кроком 0.05
results = np.zeros((len(c_values), len(d_values)), dtype=int)

# Основний алгоритм
for i, c_threshold in enumerate(c_values):
    for j, d_threshold in enumerate(d_values):
        # Розрахунок відношення
        concordance = [[calc_concordance(row, col) if row != col else 0 for col in range(len(matrix))]
                       for row in range(len(matrix))]
        discordance = [[calc_discordance(row, col) if row != col else 1 for col in range(len(matrix))]
                       for row in range(len(matrix))]
        relation = [[1 if concordance[row][col] >= c_threshold and discordance[row][col] <= d_threshold
                     else 0 for col in range(len(matrix))] for row in range(len(matrix))]

        # Перевірка циклічності та обчислення ядра
        if not check_if_cyclic(relation):
            S = get_Neumann_Morgenstern_S(relation)
            Q = get_Neumann_Morgenstern_Q(relation, S)  # Оновлена функція, що додає +1 до індексів
            results[i, j] = len(Q[-1])  # Кількість елементів ядра
        else:
            results[i, j] = 0  # Для циклічних відношень ядро відсутнє

# Виведення результатів
results_text = []
for i, c_threshold in enumerate(c_values):
    for j, d_threshold in enumerate(d_values):
        kernel_size = results[i, j]
        kernel_elements = [x+1 for x in range(kernel_size)]
        results_text.append(f"d = {d_threshold:.2f}, c = {c_threshold:.2f} | ядро: {kernel_elements} | розмір ядра: {kernel_size}")
...
# Створення сітки для d та c значень
plt.imshow(results, extent=[d_values[0], d_values[-1], c_values[0], c_values[-1]],
           origin='lower', cmap='viridis', aspect='auto')
plt.colorbar(label='Кількість елементів ядра')

# Додавання точок на графік з підписами
for i, c_threshold in enumerate(c_values):
    for j, d_threshold in enumerate(d_values):
        size = results[i, j]
        if size > 0:  # Тільки додати точки, де результат більше 0
            plt.scatter(d_values[j], c_values[i], color='red', s=100)  # Червона точка
            plt.text(d_values[j], c_values[i], f"({d_values[j]:.2f}, {c_values[i]:.2f}, {size})", fontsize=6, ha='right', color='black')

plt.xlabel('Порогове значення d')
plt.ylabel('Порогове значення c')
plt.title('Залежність складу ядра від порогових значень c та d')
plt.xticks(d_values)
plt.yticks(c_values)
